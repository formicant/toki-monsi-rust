...
pub struct Generator<'a> {
    ...
    words_for_prepanding: HashMap<String, Vec<&'a str>>,
    words_for_appanding: HashMap<String, Vec<&'a str>>,
}
...
impl<'a> Generator<'a> {
    pub fn create(word_list: &'a [&'a str], max_word_count: usize) -> Generator<'a> {
         Generator {
             ...
             words_for_prepanding: HashMap::new(),
             words_for_appanding: HashMap::new(),
        }
    }
    ...
    fn get_words_for_prepending(&mut self, matching_part: &str) -> &Vec<&'a str> {
        self.words_for_prepanding.entry(String::from(matching_part))
            .or_insert_with(||
                self.word_list.iter()
                    .map(|&word| word)  // ???
    ...
    fn get_words_for_appending(&mut self, matching_part: &str) -> &Vec<&'a str> {
        self.words_for_appanding.entry(String::from(matching_part))
            .or_insert_with(||
                self.word_list.iter()
                    .map(|&word| word)  // ???
